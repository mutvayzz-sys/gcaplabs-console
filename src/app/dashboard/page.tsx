  ...
  const send = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok || !res.body) {
        ...
        setMessages((prev) =>
          prev.map((m) => (m.id === assistantMsg.id ? { ...m, text: `Error: ${errText}`, isStreaming: false } : m))
        );
        return;
      }
        ...
        for (const block of blocks) {
          ...
          try {
            ...
            if (type === 'response.created' && evt.id) {
              ...
              const delta = (evt.text as string) || '';
              setMessages((prev) => prev.map((m) => (m.id === assistantMsg.id ? { ...m, text: m.text + delta } : m)));
            } else if (type === 'response.reasoning.delta') {
              const delta = (evt.text as string) || '';
              setMessages((prev) =>
                prev.map((m) => (m.id === assistantMsg.id ? { ...m, reasoning: (m.reasoning ?? '') + delta } : m))
              );
              ...
              const err = errObj?.message || 'Response failed';
              setMessages((prev) =>
                prev.map((m) => (m.id === assistantMsg.id ? { ...m, text: `Error: ${err}`, isStreaming: false } : m))
              );
            }
          } catch {
      ...
      if ((err as Error).name !== 'AbortError') {
        setMessages((prev) =>
          prev.map((m) =>
            m.id === assistantMsg.id ? { ...m, text: `Error: ${(err as Error).message}`, isStreaming: false } : m
          )
        );
      }
    } finally {
    ...
    <div className='flex h-screen flex-col bg-background text-foreground'>
      ...
      <header className='flex items-center justify-between border-b border-border px-6 py-3'>
        <div className='flex items-center gap-3'>
          <h1 className='font-display text-lg font-semibold'>{branding.appName}</h1>
      ...
      <div className='flex flex-1 overflow-hidden'>
        ...
        <aside className='w-64 border-r border-border bg-card p-3 overflow-y-auto'>
          ...
          <div className='space-y-1'>
            ...
            {sessions.length === 0 && (
              <p className='px-3 py-2 text-xs text-muted-foreground'>No conversations yet. Start chatting!</p>
            )}
          </div>
        </aside>
          ...
          <div className='flex-1 overflow-y-auto px-6 py-4'>
            {messages.length === 0 && (
              <div className='flex h-full items-center justify-center text-center'>
                <div>
                  <h2 className='font-display text-2xl font-semibold text-foreground'>Welcome to {branding.appName}</h2>
            ...
            {messages.map((msg) => (
              <div key={msg.id} className={`mb-4 flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div
                  ...
                  style={
                    msg.role === 'user' ? { backgroundColor: 'var(--primary)', color: 'var(--primary-foreground)' } : {}
                  }
                >
                  {msg.reasoning && (
                  ...
                  <div className='whitespace-pre-wrap text-sm'>
                    {msg.text ||
                      (msg.isStreaming && <Loader2 className='inline h-4 w-4 animate-spin text-muted-foreground' />)}
                  </div>
                </div>
              </div>
            ))}
