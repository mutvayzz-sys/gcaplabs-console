#!/usr/bin/env bash
# Register your published image as an Runtime Provider template, then spawn an instance from it.
#
# Prereqs:
#   - curl and jq installed
#   - your image published to a PUBLIC registry (push to main; make the GHCR package public)
#   - RUNTIME_API_KEY and IMAGE_REF set (see .env.example)
set -euo pipefail

API="${RUNTIME_API_BASE_URL:-https://api.runtime-provider.example}"
TEMPLATE="${TEMPLATE_NAME:-my-custom-agent}"
: "${RUNTIME_API_KEY:?Set RUNTIME_API_KEY (sk_live_...). See .env.example.}"
...
Confirm your baked-in CLI shipped (control-plane exec — no model needed):
  curl -sS -X POST ${API}/v1/instances/${id}/exec \\
    -H "Authorization: Bearer \$RUNTIME_API_KEY" -H "Content-Type: application/json" \\
    -d '{"command":"cowsay hello from my own image"}'

See your seeded skill:
  curl -sS -X POST ${API}/v1/instances/${id}/exec \\
    -H "Authorization: Bearer \$RUNTIME_API_KEY" -H "Content-Type: application/json" \\
    -d '{"command":"ls ~/.runtime/skills"}'

Delete it when you are done (this stops billing):
  curl -sS -X DELETE ${API}/v1/instances/${id} -H "Authorization: Bearer \$RUNTIME_API_KEY"
EOF
